...
//
// 12/23/2017 kvitekp -- added SetNoteVelocity()

#ifndef MIDIGRIDS_NOTE_STACK_H_
#define MIDIGRIDS_NOTE_STACK_H_
...
class NoteStack {
 public:
  ...
  void SetNoteVelocity(uint8_t note, uint8_t velocity) {
    uint8_t current = root_ptr_;
    while (current) {
     if (pool_[current].note == note) {
       pool_[current].velocity = velocity;
       break;
     }
     current = pool_[current].next_ptr;
    }
  ...
  const uint8_t note_index(uint8_t note) const {
    uint8_t current = root_ptr_;
    while (current) {
     if (pool_[current].note == note) {
       break;
